...
var helpers = {
    ...
    get_app_version: async (selection, dbtable) => {
        let qb = await pool.get_connection();
        let response = await qb
            .select(selection)
            .get(config.table_prefix + dbtable);
    ...
    generateCourseProgressData: async (duration) => {
        let data = [];
        ...
        if (duration === "monthly") {
            ...
            const endDate = currentDate.clone().endOf("year");
